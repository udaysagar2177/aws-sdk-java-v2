...
import static software.amazon.awssdk.services.s3.checksums.ChecksumsEnabledValidator.getObjectTrailingChecksumsEnabled;
import static software.amazon.awssdk.services.s3.checksums.ChecksumsEnabledValidator.putObjectTrailingChecksumsEnabled;
import static software.amazon.awssdk.services.s3.checksums.ChecksumsEnabledValidator.trailingChecksumsEnabled;
...
public final class SyncChecksumValidationInterceptor implements ExecutionInterceptor {
    ...
    public Optional<RequestBody> modifyHttpContent(Context.ModifyHttpRequest context,
        ...
        boolean checksumValidationEnabled = putObjectTrailingChecksumsEnabled(SYNC,
                                                                              executionAttributes,
                                                                              context.httpRequest().headers());

        if (context.request() instanceof PutObjectRequest && checksumValidationEnabled && context.requestBody().isPresent()) {
    ...
    public Optional<InputStream> modifyHttpResponseContent(Context.ModifyHttpResponse context,
        ...
        if (context.request() instanceof PutObjectRequest && getObjectTrailingChecksumsEnabled(context.httpResponse())
